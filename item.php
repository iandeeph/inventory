			...
			<form action="#" method="post" enctype="multipart/form-data">
				...
				<table class="striped responsive-table col s12">
					...
					<tbody>
						<?php
							$itemQry = "";
							$itemQry = "SELECT *, DATE_FORMAT(dateIn, '%e %b %Y') as tglMasuk FROM item WHERE idinventory LIKE '%".$detail."%' ORDER BY LENGTH(idinventory), idinventory";
					...
					<form action="#" method="post" enctype="multipart/form-data">
						<div class="input-field col s12 m6 l6">
							<select id="trxOutIdInventory" name="trxOutIdInventory">
								...
								<?php
									$invQry = "";
									$invQry = "SELECT idinventory FROM item WHERE status = 'Stock' AND idcategory = '".$idCat."' ORDER BY LENGTH(idinventory), idinventory";
					...
					<form action="#" method="post" enctype="multipart/form-data">
						<div class="input-field col s12 m6 l6">
							<select id="trxInIdInventory" name="trxInIdInventory">
								...
								<?php
									$invQry = "";
									$invQry = "SELECT iduser, idinventory FROM item WHERE status != 'Stock' AND idcategory = '".$idCat."' ORDER BY LENGTH(idinventory), idinventory";
									if($resultInv = mysql_query($invQry)){
										if (mysql_num_rows($resultInv) > 0) {
											while ($rowInv = mysql_fetch_array($resultInv)) {
